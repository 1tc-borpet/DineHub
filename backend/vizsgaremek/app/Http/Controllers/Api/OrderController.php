...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // (csak akkor, ha QR kódos rendelésnél van asztal megadva)
            ...
            return response()->json($order->load('items.menuItem'), Response::HTTP_CREATED);
    ...
    public function updateStatus(Request $request, string $id)
    {
        try {
            ...
            $validated = $request->validate([
                'status' => 'required|in:pending,confirmed,preparing,ready,delivered,served,completed,cancelled',
            ]);
            ...
            return response()->json($order->load('items.menuItem', 'restaurant'), Response::HTTP_OK);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return response()->json([
                'success' => false,
                'message' => 'A rendelés nem található.',
    ...
    /**
     * Create orders from cart (multiple restaurants)
     */
    public function checkoutCart(Request $request)
    {
        try {
            ...
            $itemsByRestaurant = $cart->items->groupBy('restaurant_id');
            $createdOrders = [];

            foreach ($itemsByRestaurant as $restaurantId => $items) {
                $orderNumber = 'ORD-' . date('YmdHis') . '-' . \Illuminate\Support\Str::random(6);
                ...
                $createdOrders[] = $order->load('items.menuItem');
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Rendelések sikeresen létrehozva.',
            ...
            return response()->json([
                'success' => false,
                'message' => 'Hiba a rendelések létrehozásakor.',
                'error' => $e->getMessage(),
            ], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
