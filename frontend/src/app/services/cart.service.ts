...
export class CartService {
  ...
  updateQuantity(menuItemId: number, quantity: number, restaurantId: number = 1): void {
    ...
    if (cartItem?.id) {
      this.http
        .patch(`${environment.apiUrl}/cart/items/${cartItem.id}`, { quantity })
        .subscribe({
          error: (err) => console.error('Hiba a mennyiség frissítésekor:', err),
        });
    }
  }
  ...
  removeFromCart(menuItemId: number, restaurantId: number = 1): void {
    ...
    if (cartItem?.id) {
      this.http
        .delete(`${environment.apiUrl}/cart/items/${cartItem.id}`)
        .subscribe({
          error: (err) => console.error('Hiba a tétel törlésekor:', err),
        });
    }
  }
